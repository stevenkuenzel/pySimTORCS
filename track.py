...
from segments import ConnectingSegment, CoordinateSegment, EdgeSegment, Segment, Turn, TurnDirection
...
class Track:
    ...
    def add_segment(self, length: float, width: float, angle: float):
        new_segment : CoordinateSegment = CoordinateSegment(length, width, angle, self.prev_segment)
        edge_segment : EdgeSegment | None  = new_segment.initialize(self.prev_segment.width_end if self.prev_segment is not None else self.start_width)
    ...
    def add_turn(self, angle_in_rad :float, radius_start:float, radius_end:float):
        ...
        for _ in range(self.level_of_detail):
            self.add_segment(segment1_length / self.level_of_detail, self.start_width, angle_half / self.level_of_detail)
        
        # Second half.
        for _ in range(self.level_of_detail):
            self.add_segment(segment2_length / self.level_of_detail, self.start_width, angle_half / self.level_of_detail)

    def finalize(self):
        first_segment : Segment = self.segments[0]
        last_segment : Segment = self.segments[-1]

        connecting_segment : ConnectingSegment = ConnectingSegment(last_segment, first_segment)
        connecting_segment.id = self.__next_segment_id()
        self.segments.append(connecting_segment)

        self.starting_point = first_segment.center_start + first_segment.segment_direction * (0.1 * first_segment.length_measured)
    ...
    def __find_turn_segments(self):
        ...
        for segment in self.segments:
            if isinstance(segment, CoordinateSegment):
                ...
                if segment.turn_direction != TurnDirection.Straight and \
                abs(segment.turn_angle) >= self.min_turn_rad and \
                segment.length_measured <= 50: #<-- length measured is not sqr!
...
def import_from_torcs(name, level_of_detail:int=1) -> Track:
    ...
    for x_track_segment in x_track_segments.findall('section'):
        ...
        if map_vals['type'] == 'str':
            ...
            angle_in_rad = arc * (math.pi / 180.0) * (-1.0 if left_turn else 1.0) * (-1.0 if invert else 1.0)
